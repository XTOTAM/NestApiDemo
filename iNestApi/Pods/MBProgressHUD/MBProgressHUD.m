...
static MBProgressHUD *p = nil;
static UIView *lastView = nil;

+(MBProgressHUD *)instance{
    return p;
}
...
+(MBProgressHUD *)show {
    ...
    lastView = [UIApplication sharedApplication].keyWindow;
    p = [MBProgressHUD showHUDAddedTo:[UIApplication sharedApplication].keyWindow animated:YES];
    p.userInteractionEnabled = YES;
    p.mode = MBProgressHUDModeIndeterminate;
    return p;
}
...
+(void)hide {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(lastView && lastView != view){
            [MBProgressHUD hideHUDForView:lastView animated:NO];
        }
        lastView = nil;
    });
}
...
+(MBProgressHUD *)showModeText:(NSString *)text {
    if(p){
        [p hideAnimated:NO];
    }
    lastView = [UIApplication sharedApplication].keyWindow;
    ...
    p.userInteractionEnabled = YES;
    p.detailsLabel.text = [NSString stringWithString:text];
    //p.labelText = [NSString stringWithString:text];
    p.minShowTime = 3.0f;
    [p hideAnimated:YES afterDelay:2];
    [MBProgressHUD performSelector:@selector(hide) withObject:nil afterDelay:2];
